...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Basic text input
		case 'text':
			$output .= '<input id="' . esc_attr( $value['id'] ) . '" '.(isset($value['placeholder']) ? 'placeholder="'.$value['placeholder'].'"' : '').' class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="text" value="' . esc_attr( $val ) . '" />';
			break;
		...
		case 'description_box':
			$output .= '<div class="custom_notification">' . esc_textarea( $val ) . '</div>';
			break;

		// customizer_notification
		case 'customizer_notification':
			$output .= '<div class="customizer_desc">This theme uses the native WordPress Customizer to provide you with direct control of your theme. To change colors/fonts and other aesthetic properties go to Appearance -> Customize or <a href="customize.php">click here</a></div>';
			break;
