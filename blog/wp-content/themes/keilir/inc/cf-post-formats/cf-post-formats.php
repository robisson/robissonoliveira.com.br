<?php
	...
	function cfpf_add_meta_boxes($post_type) {
		if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
			// assets
			wp_enqueue_script('cf-post-formats', get_template_directory_uri().'/inc/cf-post-formats/js/admin.js', array('jquery'), KEILIR_VERSION);
			wp_enqueue_style('cf-post-formats', get_template_directory_uri().'/inc/cf-post-formats/css/admin.css', array(), KEILIR_VERSION, 'screen');
