...
language: pt-BR
---
**TLDR:** O hedging pattern aumenta resiliência quando usa redundância especulativa para reduzir a exposição da aplicação à tail latency. A ideia é disparar uma tentativa primária, esperar um limite baseado no comportamento esperado da operação e, se ela entrar na cauda, enviar uma segunda tentativa por outro caminho plausivelmente saudável. A aplicação usa a primeira resposta válida e cancela ou limita o custo da tentativa perdedora. Já adianto que veremos muitos números no artigo e um pouco de matemática(espero que corretamente).
...
![Imagem 1 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-13.png)
...
![Imagem 2 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-02.png)
...
![Imagem 3 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-08.png)
...
![Imagem 4 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-12.png)
...
![Imagem 5 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-01.png)
...
![Imagem 6 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-09.png)
...
![Imagem 7 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-03.png)
...
![Imagem 8 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-05.png)
...
![Imagem 9 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-06.png)
...
![Imagem 10 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-10.png)
...
```python
# Hedge sem controle/limite: dispara os dois SEMPRE, sem freio, sem deadline, sem cancelar
async def get(urls):
    tarefas = [asyncio.create_task(client.get(u)) for u in urls]  # dispara TODAS
    feitas, _ = await asyncio.wait(tarefas, return_when=asyncio.FIRST_COMPLETED)
    return feitas.pop().result()
    # três problemas: (1) dobra a carga em 100% do tráfego, não nos 5% lentos;
    # (2) ninguém cancela o perdedor → vaza conexão;
    # (3) uma falha rápida vira "vencedora" e encerra a requisição boa que vinha atrás.
```
...
class HedgeBudget:
    """
    Token bucket baseado na RAZÃO real de tráfego — o freio do budget.

    Cada requisição lógica bem-sucedida deposita 'ratio' fichas; cada hedge
    disparado saca 1. Com budget_percent=10, no equilíbrio os hedges não
    passam de 1 em cada 10 requisições, seja o serviço a 5 rps ou a 50.000.

    Efeito de segunda ordem — exatamente o objetivo do budget: num incidente,
    os sucessos param de chegar, o balde para de encher, e os hedgings saturam
    no teto que você autorizou (ex.: +10%) em vez de dobrarem a carga.
    Single-threaded sob asyncio: não precisa de lock.
    """
    def __init__(self, budget_percent: float = 10.0, burst: int = 10):
        self.ratio = budget_percent / 100.0
        self.capacity = float(burst)     # rajada máxima de hedges "guardados"
        self.tokens = float(burst)

    def on_success(self) -> None:
        """Toda requisição lógica que deu certo credita o balde."""
        self.tokens = min(self.capacity, self.tokens + self.ratio)

    def try_take(self) -> bool:
        """Tenta gastar 1 ficha para hedgear. Balde vazio → NÃO dispara."""
        if self.tokens >= 1.0:
            self.tokens -= 1.0
            return True
        return False   # degrada graciosamente, sem amplificar
```
...
![Imagem 11 do artigo](/blog/2026-07-29-resiliencia-alem-do-obvio-hedging-pattern/hedging-pattern/pt/image-11.png)
