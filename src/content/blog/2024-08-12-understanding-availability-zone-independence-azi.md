...
![Article image 3](/blog/availability-zone-independence-azi/image-03.png)
...
![Article image 4](/blog/availability-zone-independence-azi/image-04.png)
...
![Article image 5](/blog/availability-zone-independence-azi/image-05.png)

Then I disabled one AZ, leaving only 2 AZs enabled, and ran the same query again:

![Article image 6](/blog/availability-zone-independence-azi/image-06.png)
...
![Article image 8](/blog/availability-zone-independence-azi/image-08.png)
...
![Article image 9](/blog/availability-zone-independence-azi/image-09.png)
...
![Article image 10](/blog/availability-zone-independence-azi/image-10.png)

In the image below, the traffic from the failing AZ has already been evacuated, and only the other AZs that are operating normally continue receiving traffic. Of course, in this specific architecture, we still have some losses. Because we are using RDS, it depends a little on whether you are using a [multi-AZ DB instance or a multi-AZ DB cluster](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.MultiAZ.html). One of the advantages is having write instances, write instances with read replicas, and standby instances that can be configured to become primary instances. This process can take a few seconds and, in this case, AZI will not solve everything, but it will help a lot. When an AZ that contains the writer instance has a problem, evacuating traffic from that AZ is one step, and another step is performing the RDS failover.

![Article image 11](/blog/availability-zone-independence-azi/image-11.png)

The first step in our architecture to reduce cross-AZ traffic is to make sure our traffic entry point does not perform cross-AZ traffic, in this case the ALB. You can do that by [disabling cross-zone traffic for target groups](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/disable-cross-zone.html). The same can also be done with [Network Load Balancer](https://docs.aws.amazon.com/elasticloadbalancing/latest/network/target-group-cross-zone.html) or [Classic Load Balancer](https://docs.aws.amazon.com/elasticloadbalancing/latest/classic/enable-disable-crosszone-lb.html#disable-cross-zone). With that, whenever a request reaches an ALB node, it will redirect to instances that are in the same zone as that node. In this simple architecture, which is very common, this configuration adjustment already causes the effect shown below.

![Article image 12](/blog/availability-zone-independence-azi/image-12.png)

But disabling cross-AZ traffic in the ALB has pros and cons. The pros are:

- It makes it easier to identify [gray failures](https://docs.aws.amazon.com/whitepapers/latest/advanced-multi-az-resilience-patterns/gray-failures.html#:~:text=Gray%20failures%20are%20defined%20by,entities%20observe%20the%20failure%20differently.), intermittent failures, or failures that can only be identified from some perspectives and not others. For example, a component that fails when a customer tries to use the application, but the monitoring system cannot identify it because it happens in a very specific percentage or situation.
...
The main downside is that with cross-AZ traffic disabled you no longer have traffic equally balanced across all instances of your application. Learn more [here](https://docs.aws.amazon.com/elasticloadbalancing/latest/userguide/how-elastic-load-balancing-works.html). If you are using two AZs, traffic will still reach the ALB in a balanced way, but distribution to your targets and instances will not necessarily be balanced, as shown below:

![Article image 13](/blog/availability-zone-independence-azi/image-13.png)

In this case, you need extra attention to make sure all your AZs have the same number of instances, something [AWS Auto Scaling](https://aws.amazon.com/autoscaling/) helps with a lot.
...
To access IMDS, you can make an HTTP request to the endpoint [http://169.254.169.254/.](http://169.254.169.254/) To get the Availability Zone (AZ) ID where the instance is operating, you can make a specific request to the following endpoint:
...
![Article image 15](/blog/availability-zone-independence-azi/image-15.png)
...
![Article image 16](/blog/availability-zone-independence-azi/image-16.png)
...
![Article image 17](/blog/availability-zone-independence-azi/image-17.png)

If the names above look a little strange to you, I suggest reading this excellent article from the Amazon Builders' Library: [Automating safe, hands-off deployments](https://aws.amazon.com/builders-library/automating-safe-hands-off-deployments/?nc1=h_ls).

If we go a little deeper into each deployment event, it is also common to see approaches such as [blue/green deployment](https://docs.aws.amazon.com/whitepapers/latest/overview-deployment-options/bluegreen-deployments.html), where you have two equal environments or copies, blue and green, and one is always active, initially blue for example, receiving traffic from your customers. When you need to start a new version, you deploy it to the environment that is not being used, in this case green. Another well-known approach is [canary deployment](https://docs.aws.amazon.com/whitepapers/latest/overview-deployment-options/canary-deployments.html), where you also have two environments, but each new version is made available in a parallel environment and you gradually shift your customers' traffic. You can choose to move 10% of your traffic to the new version, then 50%, and then 100%.

![Article image 18](/blog/availability-zone-independence-azi/image-18.png)
...
![Article image 19](/blog/availability-zone-independence-azi/image-19.png)
...
![Article image 20](/blog/availability-zone-independence-azi/image-20.png)
...
![Article image 21](/blog/availability-zone-independence-azi/image-21.png)
...
![Article image 24](/blog/availability-zone-independence-azi/image-24.png)

Once the alarms return to normal state, the same process can be triggered to reverse the AZ evacuation:

![Article image 25](/blog/availability-zone-independence-azi/image-25.png)
...
A more robust approach would use only data plane APIs and resources to perform zone evacuation and restoration. This approach makes heavy use of the [Route 53 Health Checks](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/dns-failover.html) feature. Taking the documentation as reference, Amazon Route 53 health checks monitor the health and performance of applications. Each health check you create can monitor one of the following:
...
![Article image 26](/blog/availability-zone-independence-azi/image-26.png)
...
![Article image 27](/blog/availability-zone-independence-azi/image-27.png)
...
Lastly, I want to talk about a more managed approach to evacuating traffic from an AZ, using the [Route 53 Application Recovery Controller](https://docs.aws.amazon.com/r53recovery/latest/dg/what-is-route53-recovery.html) service feature called [zonal shift](https://docs.aws.amazon.com/r53recovery/latest/dg/arc-zonal-shift.html). From the AWS documentation, Amazon Route 53 Application Recovery Controller (Route 53 ARC) helps you prepare and complete faster recovery for applications running on AWS. Route 53 ARC provides two feature sets: multiple Availability Zone (AZ) recovery, which includes zonal shift and zonal autoshift, and multi-Region recovery, which includes routing control and readiness check.

![Article image 28](/blog/availability-zone-independence-azi/image-28.png)
...
- [https://aws.amazon.com/blogs/containers/choosing-container-logging-options-to-avoid-backpressure/](https://aws.amazon.com/blogs/containers/choosing-container-logging-options-to-avoid-backpressure/)
