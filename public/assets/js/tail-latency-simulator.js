(function () {
  function mountSimulator() {
    ...
    shell.innerHTML = `
      <header class="tail-simulator__header">
        <div>
          <p class="tail-simulator__eyebrow">Simulador</p>
          <h3 id="tail-simulator-title">Tail latency, fan-out e hedging</h3>
        </div>
        <button class="tail-simulator__button" type="button" data-action="play" aria-pressed="false">Play</button>
      </header>
      ...
      <div class="tail-simulator__metrics" aria-live="polite">
        <div><strong data-metric="risk">63,4%</strong><span>Pelo menos um straggler</span></div>
        <div><strong data-metric="p99Base">-</strong><span>p99 sem hedging</span></div>
        <div><strong data-metric="p99Hedge">-</strong><span>p99 com hedging</span></div>
        <div><strong data-metric="extraLoad">-</strong><span>Carga extra</span></div>
      </div>
      <div class="tail-simulator__views">
        <figure class="tail-simulator__panel">
          <figcaption>Uma requisição com fan-out</figcaption>
          <canvas width="760" height="300" data-canvas="timeline"></canvas>
        </figure>
        <figure class="tail-simulator__panel">
          <figcaption>Percentis simulados em 8.000 requisições</figcaption>
  ...
  function drawTimeline(params) {
    ...
    drawText(ctx, `requisição simulada: ${formatMs(result.base)} sem hedge / ${formatMs(result.hedged)} com hedge`, 14, 18, {
      color: "#252d35",
    ...
    drawText(ctx, "hedge", left + params.hedgeAfterMs * scale, height - 8, { color: "#747d86", align: "center" });
    drawText(ctx, "sem", left + result.base * scale, height - 8, { color: "#f69087", align: "center" });
    drawText(ctx, "com", left + result.hedged * scale, height - 8, { color: "#85a9b3", align: "center" });
    ...
    calls.forEach((call, index) => {
      ...
      if (index < 8 || index === calls.length - 1) {
        drawText(ctx, `c${index + 1}`, 14, y + rowHeight - 2, { color: "#747d86", font: "10px Helvetica, Arial, sans-serif" });
      }
    });

    if (params.fanout > calls.length) {
      drawText(ctx, `mostrando ${calls.length} de ${params.fanout} chamadas`, 14, height - 8, {
        color: "#747d86",
  ...
  function drawPercentiles(stats) {
    ...
    drawText(ctx, "sem hedging", left + 14, height - 7, { color: "#747d86", font: "10px Helvetica, Arial, sans-serif" });
    ctx.fillStyle = "#85a9b3";
    ctx.fillRect(left + 96, height - 10, 10, 3);
    drawText(ctx, "com hedging", left + 110, height - 7, { color: "#747d86", font: "10px Helvetica, Arial, sans-serif" });
  }
  ...
  function render() {
    ...
    outputs.fanout.textContent = `${params.fanout} chamada${params.fanout === 1 ? "" : "s"}`;
